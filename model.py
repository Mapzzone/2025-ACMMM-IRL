...
class embed_net(nn.Module):
    def __init__(self, class_num, no_local='on', gm_pool='on', arch='resnet50'):
        ...
        self.LPA0 = LPA(in_channel=64)
        self.LPA1 = LPA(in_channel=256)
        self.LPA2 = LPA(in_channel=512)

        self.NAF1 = NAFBlock(c=1024)
        self.NAF2 = NAFBlock(c=1024)
        self.NAF3 = NAFBlock(c=1024)

        self.delta = nn.Parameter(torch.zeros((1, 1024, 1, 1)), requires_grad=True)
        self.sin = nn.Parameter(torch.zeros((1, 1024, 1, 1)), requires_grad=True)
    ...
    def forward(self, x1_1, x1_2, x2_1, x2_2, modal=0):
        ...
        if self.non_local == 'on':
            ...
            for i in range(len(self.base_resnet.base.layer3)):
                ...
                if i == self.NL_3_idx[NL3_counter]:
                    ...
                    NL3_counter += 1  # [96, 1024, 24, 12]

            # Layer 4
            xlay3 = x
            x1 = self.layerATT0(x, xlay0, 0)
            # x1 = (x1 + self.NAF1(x) * 0.3)
            x1 = (x1 + self.NAF1(x)*self.delta)

            x2 = self.layerATT1(x1, xlay1, 1)
            x2 = (x2 + self.NAF2(x) *  0.3)

            x3 = self.layerATT2(x2, xlay2, 2)
            x = self.layerATT3(x3, xlay3, 3)
        ...
        if self.training:
            # if use complementary_Learning_Loss
            # losss4 = self.complementary_Learning_Loss(xlay4)
            # return x_pool, self.classifier(feat), losss4
