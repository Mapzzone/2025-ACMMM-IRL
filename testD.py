...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            feat_pool1, feat_fc1 = net(input1,input1,input1, input1, test_mode[0])
            feat_pool2, feat_fc2 = net(input2,input2,input2, input2, test_mode[0])
            feat = feat_pool1 + feat_pool2
            feat_att = feat_fc1 + feat_fc2
            gall_feat_pool[ptr:ptr + batch_num, :] = feat[:batch_num].detach().cpu().numpy()
            gall_feat_fc[ptr:ptr + batch_num, :] = feat_att[:batch_num].detach().cpu().numpy()

            ptr = ptr + batch_num
            gall_feat_label.extend(label.detach().cpu().numpy())
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
    return gall_feat_pool, gall_feat_fc,np.array(gall_feat_label)
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(query_loader):
            ...
            input1 = Variable(input.cuda())
            input2 = Variable(fliplr(input).cuda())
            feat_pool1, feat_fc1 = net(input1,input1,input1, input1, test_mode[1])
            feat_pool2, feat_fc2 = net(input2,input2,input2, input2, test_mode[1])
            feat = feat_pool1 + feat_pool2
            feat_att = feat_fc1 + feat_fc2
            query_feat_pool[ptr:ptr + batch_num, :] = feat[:batch_num].detach().cpu().numpy()
            query_feat_fc[ptr:ptr + batch_num, :] = feat_att[:batch_num].detach().cpu().numpy()

            ptr = ptr + batch_num  
            query_feat_label.extend(label.detach().cpu().numpy())       
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
    return query_feat_pool, query_feat_fc,np.array(query_feat_label)
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        test_trial = trial + 1
        model_path = checkpoint_path + 'regdb_awg_p4_n6_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
    ...
    query_feat_pool, query_feat_fc,query_label= extract_query_feat(query_loader)
    for trial in range(10):
        ...
        #if test_mode == [1, 2]:
        if 1:
            distmat1 = np.matmul(query_feat_pool, np.transpose(gall_feat1))
            distmat2 = np.matmul(query_feat_fc, np.transpose(gall_feat2))
